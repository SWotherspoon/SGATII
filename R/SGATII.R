...
##'
##' This sampler does not require initial location estimates; it only
##' uses the initial locations `x0` from the model to specify fixed
##' points.
##' 
...
heliosDiscSampler <- function(weights,x=NULL,iters=100L) {
  ...
  logp.p <- model$logp.p
  fixedx <- model$fixedx
  x0 <- if(is.list(model$x0)) model$x0[[1]] else model$x0
