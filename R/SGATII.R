...
##' @export
extractHeliosDiscRasterWeights <- function(raster,weights,k,log=TRUE) {
...
##'
##' The [heliosDiscRasterWeights()] function stores rasters in a
...
##' raster form.
##' 
##' @title Uncompress raster vector
##' @param raster a template raster
##' @param v the vector of finite values
##' @return the vector in raster form.
##' @export
uncompressRaster <- function(raster,v) {

  r <- deepcopy(raster)
  if(hasValues(r))
    values(r)[is.finite(values(r))] <- v
  else
    values(r) <- v
  r
}
...
##' @param weights a weights object generated with
##'   `heliosDiscGridWeights` or `heliosDiscRasterWeights`
##' @param gamma regularization parameter controlling the distance
##'   cost.
##' @param ngbrs the number of neighbours of each grid point to
##'   consider.
##' @param debug should extra debugging information be returned.
##' @return By default the track locations as a two column matrix. If
##'   `debug` is true, the track and data matrices generated by the
##'   dynamic programming algorithm are returned as a list.
##' @export
heliosDiscMinimalTrack <- function(weights,gamma,ngbrs=50,debug=FALSE) {
  ...
  if(debug) MaxL <- matrix(0,nrow(W),ncol(W))
  ...
  ## Coords of path
  x <- grid[p,]
  if(debug) list(x=x,K=K,D=D,MaxL=MaxL) else x
}
