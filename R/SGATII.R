...
##' tag is stationary. If `autoscale` is `NA` the scaling of the
##' weights is determined by the obscuration penalty chosen for the
##' model. If `autoscale` is a constant the weights are scaled so the
...
##' @param model a model structure as generated by [`heliosModel()`]
##'   or equivalent.
##' @param wmin a lower bound for the (finite) weights
...
##' @param iters number of samples to draw.
##' @return The functions `heliosDiscGridWeights` and
##'   `heliosDiscRasterWeights` return a list containing the matrix of
##'   grid points, the model object and the matrix of weights, and
##'   `heliosDiscSampler` returns an n x 2 x r array of r samples for
##'   the n locations.
##' @importFrom stats runif
##' @export
...
##' @export
heliosDiscGridWeights <- function(grid,model,autoscale=NA) {
...
##' @importFrom terra hasValues values xyFromCell ncell
##' @export
heliosDiscRasterWeights <- function(raster,model,autoscale=NA) {
  ...
  heliosDiscGridWeights(grid,model,autoscale)
}
